        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="PATCH" action="{{ route('block.update', $block) }}">
                        @csrf
                        @method('PATCH')
                        <div class="mb-4">
                        ...
                        <div x-data="{
                                        ...
                                        tinymce.init({
                                            ...
                                            setup: function(editor) {
                                                editor.on('change', function() {
                                                    let input = e.parentNode.querySelector('[some]');
                                ...
                                <tbody @dragover.prevent
                                    ...
                                    <template x-for="(field, index) in fields" :key="index">
                                        <tr class="row">
                                            ...
                                            <td width="16%">
                                                <select class="border border-gray-300 w-full" name="type"
                                            ...
                                            <template x-if="field.type === 'image'">
                                                <td width="80%">
                                                    ...
                                                    <input type="hidden" name="content" some="content"
                                                        x-model="field.content" data-image>
                                                </td>
                                            </template>
                                            <template x-if="field.type === 'link'">
                                                <td width="80%">
                                                    <input type="url" x-model="field.content"
                                            ...
                                            <template x-if="field.type === 'wysiwyg'">
                                                <td width="80%">
                                                    ...
                                                    <input type="hidden" name="content" some="content"
                                                        x-model="field.content" data-content>
                                                </td>
                                            </template>
                                            <td class="text-right " width="17%">
                                                <button @click="fields.splice(index, 1)" class="border-red-100 px-4 py-2 border border-gray-300">Remove</button>
                                            </td>
                                        </tr>
                                    </template>
                                </tbody>
                            ...
                            <div class="mb-4 cursor-pointer flex-grow flex-1">
                                <span class="border bg-gray-800 text-white px-4 py-2 text-xs"
                                    @click="saveFields()">Save</span>
                            </div>
                            <div :class="gallery ? 'relative z-10' : 'hidden'" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                                                    ...
                                                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                                        ...
                                                        <div class="mt-2 grid grid-cols-4 gap-4 gallery-container">
                                                            @forelse ($files as $media) 
                                                                <div class="w-20">
                                                                    <img @click="(s) => {console.log(fields[currentRow].content);fields[currentRow].content = s.target.src;s.target.classList.toggle('border-indigo-400')}" src="{{ $media->path }}" alt="{{ $media->name }}" class="mb-2 h-full object-cover border-2"> {{-- Added margin --}}
                                                                </div>
                        ...
                        <div class="mt-6">
                            <button type="submit"
                                class="px-4 py-2 bg-gray-800 text-white rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                Update
                            </button>
                        </div>
                    </form>
